...
- Stores last price + timestamp into Redis
...
async def openai_analyze(price_map: dict):
    ...
    try:
        # Use ChatCompletion via openai SDK in a thread to avoid blocking
        model = "gpt-3.5-turbo"
...
async def periodic_task():
    ...
    try:
        ...
        # keep running but without redis write
        r = None
        ...
        while True:
            ...
            for s, p in zip(SYMBOLS, fetched):
                if p is not None:
                    ...
                    # try reading previous value from redis if fetch failed
                    if r:
